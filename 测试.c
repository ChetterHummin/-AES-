...
int** AddRounKey_0(int** plaintext_0, int** W, int length, int i) {
...
int** SubBytes(int** plaintext_0, int length,int time) {
    int m, n;
    for (int i = 0; i < 4; i++) {
        for (int j = time*4; j < (time+1)*4; j++) {
            m = plaintext_0[i][j] / 16;
            n = plaintext_0[i][j] % 16; //printf("[%d  %d]",m,n);
...
int** ShiftRows(int** plaintext_0, int length,int time) {
...
int** MixColumns(int** plaintext_0, int length,int time) {
    int sum = 0;
    int sum0, sum1, sum2, sum3;
    int Sum[4] = { 0 };
    int** plaintext_1 = NULL; printf(" MixColumns%d",length);
...
int** AddRounKey(int** plaintext_0, int** W, int length, int i,int time) {
...
int** jiami_0(int** plaintext_0, int** W, char* plaintext,int i) {
    ...
    for (int time = 0; time < (strlen(plaintext) / 16); time++) {
        ...
        if (i != 9) {
            MixColumns(plaintext_0, (strlen(plaintext) / 4),time);
        }
        AddRounKey(plaintext_0, W, (strlen(plaintext) / 4), i + 1,time);
    }
    return plaintext_0;
}
